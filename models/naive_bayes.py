...
class NaiveBayes:
    def __init__(self, alpha_prior=10, alpha_likelihood=1.0):
    ...
    def fit(self, X, y):
        ...
        self.class_log_prior_ = np.log((class_counts + self.alpha_prior) / (class_counts.sum() + n_classes * self.alpha_prior))
        ...
        self.feature_log_prob_ = np.log((feature_counts_per_class + self.alpha_likelihood) / 
                                  (feature_counts_per_class.sum(axis=1, keepdims=True) + X.shape[1] * self.alpha_likelihood))
